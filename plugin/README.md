...
| Alerts show only counts, `severity_unavailable: true` | Your tenant's Insights layer exposes alerts as an aggregate (counts by MU/RN/SC). On a field-verified tenant no per-alert Insights view exists at all — severity lives in the dedicated Prisma Access alerts/notifications API (not yet integrated; Phase-2 candidate) | `discover_insights(kind="alerts_detail")` checks whether *your* tenant has a detail view; if none is found this is an API limitation, not a missing setting |
