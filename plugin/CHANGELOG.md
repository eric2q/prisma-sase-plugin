...
behaves. It is released because the install path is part of the product, and
both entries below describe things you could only find out by failing.
...
- **DOC (install, high):** **the prerequisites are now install commands, not a
  ...
  looking. Nobody reads a troubleshooting grid on the way in. The section now
  opens with the commands to run (`brew install uv git`, or the two `winget`
  lines) and a single `uvx --version && git --version` that answers *am I
  ready* in one line. The reasoning and the failure signatures are still there,
  collapsed underneath, for whoever wants them afterwards.
...
- **DOC (offline, high):** **an unpinned entry does not launch without a
  ...
  were each launched with the network cut and a warm cache, and only the last
  one started.
  ...
  **install-time** requirement only. With the version already cached, launches
  never call it — uv's GitHub fast path resolves the commit over HTTPS.
  Removing git from `PATH` entirely and launching again still works.

  One collision worth naming: the offline failure shares the enclosing
  *"Git operation failed"* line with the missing-git case, but not the
  *"executable not found"* one. Three distinct failures, two overlapping
  messages — the Windows section now says which is which.
