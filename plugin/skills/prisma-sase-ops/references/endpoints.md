...
| alerts_detail | `prisma_sase_external_alerts_current` (single-segment, no view) | 🔶 named by PANW guidance (2026-07-24) as the per-alert severity view (fields incl. `alert_id`, `severity`, `severity_id`, `state`, `updated_time`) — **live-tested ONCE with a negative result**: the sg tenant returned `DATA10003` (view absent; issue #9, 2026-07-24). Kept as the leading candidate for other tenant versions. `query_alerts` still tries it automatically (one extra failing call before the aggregate fallback on tenants like sg), or probe with `discover_insights(kind="alerts_detail")` — if it fails everywhere, use the UI-capture workflow below |
