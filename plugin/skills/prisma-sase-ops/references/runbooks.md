...
1. `--selfcheck` (terminal) — confirm interpreter, packages, credentials:
   ...
   ```
   Credentials belong in the **Local MCP servers** entry named `prisma-sase`
   (written for you by `prisma-sase-setup`) or in `~/.prisma-sase.env`
   (chmod 600). Note that a selfcheck run from a plain shell **cannot see the
   entry's env** — that reaches the server process only — so a `MISSING` line
   there is expected and is not evidence of a broken install. See the Skill's
   credential-handling rules before concluding anything. On macOS, GUI apps
