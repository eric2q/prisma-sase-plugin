...
4. **Bump `version` in `.claude-plugin/marketplace.json` — both entries**
   (single source of truth for what users see; keep the two entries identical).
5. Commit + push:
   ```bash
...
> Prefer zero-ceremony releases? Delete the `version` fields from both plugin
...
## Why one code tree but two catalog entries
...
The marketplace catalog solves this: both entries point at the same
`./plugin` source with `"strict": false`, and each entry carries its own
`mcpServers` launch config (`bash mcp/run.sh` vs `cmd /c mcp\run.cmd`).
One push updates both. Do not add a `.mcp.json` or `.claude-plugin/plugin.json`
...
```bash
python3 tools/build-standalone.py        # writes dist/prisma-sase.plugin + dist/prisma-sase-windows.plugin
```
...
- [ ] `marketplace.json` version bumped in **both** entries
