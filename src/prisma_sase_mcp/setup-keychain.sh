...
echo ""
echo "== done =="
echo "Verify:  uvx --from git+https://github.com/eric2q/prisma-sase-plugin \\"
echo "             prisma-sase-mcp --selfcheck"
echo "         (expect: secret from: PRISMA_SECRET_CMD (keychain/manager-backed))"
echo "Then RESTART the app completely -- Desktop: Cmd-Q; CLI: restart claude."
echo ""
echo "This file is the fallback source. Anything set in the 'prisma-sase' Local"
echo "MCP servers entry takes precedence automatically -- values the host"
echo "injects win over this file -- so you can move the three non-secret values"
echo "there and delete them here (keep any tuning ones). Leave PRISMA_SECRET_CMD"
echo "wherever you prefer: it is a command, not the secret itself."
